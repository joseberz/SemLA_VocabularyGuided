...
ADE20K_133_CATEGORIES = [
    ...
    {"color": [82, 0, 255], "id": 101, "isthing": 0, "name": "stage"},
    {"color": [163, 255, 0], "id": 102, "isthing": 1, "name": "van"},
    {"color": [8, 184, 170], "id": 103, "isthing": 1, "name": "fountain"},
    {"color": [0, 255, 92], "id": 104, "isthing": 0, "name": "canopy"},
    {
        "color": [184, 0, 255],
        "id": 105,
        "isthing": 1,
    ...
    {"color": [255, 0, 31], "id": 106, "isthing": 1, "name": "plaything, toy"},
    {"color": [0, 214, 255], "id": 107, "isthing": 1, "name": "stool"},
    {"color": [92, 255, 0], "id": 108, "isthing": 1, "name": "basket, handbasket"},
    {"color": [70, 184, 160], "id": 109, "isthing": 1, "name": "bag"},
    {"color": [163, 0, 255], "id": 110, "isthing": 1, "name": "minibike, motorbike"},
    {"color": [71, 255, 0], "id": 111, "isthing": 1, "name": "oven"},
    {"color": [255, 0, 163], "id": 112, "isthing": 1, "name": "ball"},
    {"color": [255, 204, 0], "id": 113, "isthing": 1, "name": "food, solid food"},
    {"color": [255, 0, 143], "id": 114, "isthing": 1, "name": "step, stair"},
    {"color": [133, 255, 0], "id": 115, "isthing": 1, "name": "trade name"},
    {"color": [255, 0, 235], "id": 116, "isthing": 1, "name": "microwave"},
    {"color": [245, 0, 255], "id": 117, "isthing": 1, "name": "pot"},
    {"color": [255, 245, 0], "id": 118, "isthing": 1, "name": "bicycle"},
    {"color": [214, 255, 0], "id": 119, "isthing": 1, "name": "dishwasher"},
    {"color": [20, 0, 255], "id": 120, "isthing": 0, "name": "blanket, cover"},
    {"color": [255, 255, 0], "id": 121, "isthing": 1, "name": "sculpture"},
    {"color": [0, 153, 255], "id": 122, "isthing": 1, "name": "hood, exhaust hood"},
    {"color": [0, 41, 255], "id": 123, "isthing": 1, "name": "sconce"},
    {"color": [0, 255, 204], "id": 124, "isthing": 1, "name": "vase"},
    {"color": [41, 0, 255], "id": 125, "isthing": 1, "name": "traffic light"},
    {"color": [41, 255, 0], "id": 126, "isthing": 1, "name": "tray"},
    {"color": [173, 0, 255], "id": 127, "isthing": 1, "name": "trash can"},
    {"color": [0, 245, 255], "id": 128, "isthing": 1, "name": "fan"},
    {"color": [0, 255, 184], "id": 129, "isthing": 1, "name": "plate"},
    {"color": [25, 194, 194], "id": 130, "isthing": 1, "name": "glass, drinking glass"},
    {"color": [102, 255, 0], "id": 131, "isthing": 1, "name": "clock"},
    {"color": [92, 0, 255], "id": 132, "isthing": 1, "name": "flag"},
]
...
def get_coco_categories_with_prompt_eng():
    ...
    coco_id_names = open(os.path.join(_THIS_DIR, 'coco_panoptic_with_prompt_eng.txt')).read().splitlines()
    coco_idx = 0
...
def get_coco_categories_with_prompt_eng_split(allowed_ids: set[int]):
    COCO_CATEGORIES_ = copy.deepcopy(COCO_CATEGORIES)
    coco_id_names = open(os.path.join(_THIS_DIR, 'coco_panoptic_with_prompt_eng.txt')).read().splitlines()
    coco_idx = 0
    for line in coco_id_names:
        ...
        COCO_CATEGORIES_[coco_idx]["name"] = name
        coco_idx += 1
    COCO_CATEGORIES_ = [c for c in COCO_CATEGORIES_ if c["id"] in allowed_ids]
    return COCO_CATEGORIES_


def get_coco_stuff_categories_with_prompt_eng():
    COCO_STUFF_CATEGORIES_ = copy.deepcopy(COCO_STUFF_CATEGORIES)
    coco_id_names = open(os.path.join(_THIS_DIR, 'coco_stuff_with_prompt_eng.txt')).read().splitlines()
    coco_idx = 0
    for line in coco_id_names:
        idx, name = line.split(':')
        idx = int(idx)
...
def get_ade20k_categories_with_prompt_eng():
    ...
    ade20k_id_names = open(os.path.join(_THIS_DIR, 'ade20k_150_with_prompt_eng.txt')).read().splitlines()
    ade_idx = 0
...
def get_ade20k_133_categories_with_prompt_eng():
    ADE20K_133_CATEGORIES_ = copy.deepcopy(ADE20K_133_CATEGORIES)
    ade20k_id_names = open(os.path.join(_THIS_DIR, 'ade20k_133_with_prompt_eng.txt')).read().splitlines()
    ade_idx = 0
    for line in ade20k_id_names:
        idx, name = line.split(':')
        idx = int(idx)
        if idx == 0 or name == "invalid_class_id":
            continue
        #print(ADE20K_150_CATEGORIES_[ade_idx]["name"], '->', name)
        assert ADE20K_133_CATEGORIES_[ade_idx]["id"] == idx - 1
        ADE20K_133_CATEGORIES_[ade_idx]["name"] = name
        ade_idx += 1
    return ADE20K_133_CATEGORIES_


def get_cityscapes_categories_with_prompt_eng():
    CITYSCAPES_CATEGORIES_ = copy.deepcopy(CITYSCAPES_CATEGORIES)
    cityscapes_id_names = open(os.path.join(_THIS_DIR, 'cityscapes_with_prompt_eng.txt')).read().splitlines()
    cityscapes_idx = 0
...
def get_ade20k_847_categories_with_prompt_eng():
    ...
    ade20k_847_id_names = open(os.path.join(_THIS_DIR, 'ade20k_847_with_prompt_eng.txt')).read().splitlines()
    ade_idx = 0
...
def get_pascal_21_categories_with_prompt_eng():
    ...
    pascal_21_id_names = open(os.path.join(_THIS_DIR, 'pascal_voc_21_with_prompt_eng.txt')).read().splitlines()
    pas_idx = 0
...
def get_pascal_ctx_459_categories_with_prompt_eng():
    ...
    pascal_ctx_459_id_names = open(os.path.join(_THIS_DIR, 'pascal_ctx_459_with_prompt_eng.txt')).read().splitlines()
    pas_idx = 0
...
def get_pascal_ctx_59_categories_with_prompt_eng():
    ...
    pascal_ctx_59_id_names = open(os.path.join(_THIS_DIR, 'pascal_ctx_59_with_prompt_eng.txt')).read().splitlines()
    pas_idx = 0
...
def get_mapillary_vistas_categories_with_prompt_eng():
    ...
    mapillary_vistas_id_names = open(os.path.join(_THIS_DIR, 'mapillary_vistas_with_prompt_eng.txt')).read().splitlines()
    mapillary_idx = 0
