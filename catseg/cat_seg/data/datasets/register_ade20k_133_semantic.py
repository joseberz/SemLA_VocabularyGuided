...
ADE20K_133_CATEGORIES = openseg_classes.get_ade20k_133_categories_with_prompt_eng()
print(ADE20K_133_CATEGORIES)

ADE20k_COLORS = [k["color"] for k in ADE20K_133_CATEGORIES]

MetadataCatalog.get("a133_sem_seg_train").set(
    stuff_colors=ADE20k_COLORS[:],
)
MetadataCatalog.get("a133_sem_seg_val").set(
    stuff_colors=ADE20k_COLORS[:],
)
...
def get_metadata():
    ...
    thing_classes = [k["name"] for k in ADE20K_133_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in ADE20K_133_CATEGORIES if k["isthing"] == 1]
    stuff_classes = [k["name"] for k in ADE20K_133_CATEGORIES]
    stuff_colors = [k["color"] for k in ADE20K_133_CATEGORIES]
    ...
    for i, cat in enumerate(ADE20K_133_CATEGORIES):
        if cat["isthing"]:
...
def register_all_ade20k_semantic(root):
    #root = os.path.join(root, "ADEChallengeData2016") ADE133_ignore_thre_0_1
    root = os.path.join(root, "ADE133_ignore_thre_0_1")
    for name, dirname in [("train", "training"), ("val", "validation")]:
        image_dir = os.path.join(root, "images", dirname)
        gt_dir = os.path.join(root, "annotations_detectron2", dirname)
        name = f"a133_sem_seg_{name}"
        ...
        MetadataCatalog.get(name).set(
            stuff_classes=[x["name"] for x in ADE20K_133_CATEGORIES],
