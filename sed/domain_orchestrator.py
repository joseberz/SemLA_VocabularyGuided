...
BASE_MODEL_CONFIG = custom_domain_args(
    ...
    config_file="configs/convnextL_768.yaml",
    num_gpus=1,
...
class DomainOrchestrator:
    ...
    def zero_shot_only(self, target_domain_name: str):
        # GIANLUCA's VERSION - Something like this would be great!
        results = {}
        results.update({"zeroshot": {}})

        # Benchmark zeroshot model
        self._set_current_target_domain(
            target_domain_name,
            self._target_domains[target_domain_name].args,
            index="index-1000",  # index does not matter for zeroshot
        )
        ...
        args = custom_domain_args(
            target_args.config_file,
            Path(target_args.model_path).parent.absolute().as_posix(),
            num_gpus=1,
            model_path="models/model_final.pth",
        )
        self.current_model = self._load_base_model(args)

        result_dict = self._benchmark_on_current_target_domain(name="zeroshot")
        print(result_dict)
    ...
    def visualize_all_embeddings(self):
        self.observer.visualize_domains_pca(self._domains.values())

    def visualize_all_embeddings_tsne(self):
        self.observer.visualize_domains_tsne(self._domains.values())

    def average_pairwise_distance_between_two_domains(
        self,
        domain1_embeddings: np.typing.NDArray,
        domain2_embeddings: np.typing.NDArray,
    ):
        distances = []
        for vec1 in domain1_embeddings:
            for vec2 in domain2_embeddings:
                dist = self.distance_measure(vec1, vec2)
                distances.append(dist)
        return np.mean(distances)

    def distance_to_domains(self, dataset_name: str, dataset_path: Union[str, Path]):
        dataset_embeddings = self._embed_dataset(dataset_name, dataset_path)

        distances = []
        for domain in self._domains.values():
            print(f"Calculating distance from {dataset_name} to {domain.name} ...")

            distance = self.average_pairwise_distance_between_two_domains(
                dataset_embeddings, domain.dataset_embeddings
            )
            print(f"Distance from {dataset_name} to {domain.name}: {distance}")

            # The order as defined by DATASETS.TEST in each config file
    ...
    def trajectory_experiment(
        ...
        name: str,
        save_embeddings_path: Union[Path, str] = None,
    ):
        test_dataset = Path(test_config.dataset_path)
        test_dataset.exists()
        print("dataset path: ", test_dataset)

        self._add_target_domain(
            name="experiment",
            args=test_config,
            sort_input=True,
            save_embeddings_path=save_embeddings_path,
        )
        experiment_domain = self._domains["experiment"]

        window = deque()

        closest_domain_history = []

        embeddings = experiment_domain.dataset_embeddings

        average_embeddings = []
        for i, embdng in enumerate(embeddings):
            current_embedding = self._sliding_average(embdng, window)
            average_embeddings.append(current_embedding)

        return self.observer.draw_trajectory(
            average_embeddings, experiment_domain, name=name
        )
    ...
    # def _centroid_merge(self, target_domain_name):

    #     target_domain_prototype = self.current_target_domain.val_average_embedding

    #     domain_distance_mapping = self.observer.calcualte_distance_to_domains(
    #         target_domain_prototype,
    #         [domain for _, domain in self._source_domains],
    #     )

    #     # Save the name of the closest domain
    ...
    #     merged_name += f"_{self.merge_type}_{target_domain_name}"
    ...
    #     # A dummy name so that the merged adapter does not have the same name as any of source adapters since this adapter will be deleted later
    #     merged_name = ""
    #     for source in k_closest_names:
    #         merged_name += f"_{source}_"
    #     merged_name += f"_{self.merge_type}_{self.current_target_domain.name}"

    #     # Uniform merging
    #     weights = [1 / self.k_adapters for _ in range(self.k_adapters)]
    ...
    #     # Activate the merged adapter after creating it
    #     print(f"Setting {merged_name} as the active adapter.\n")
    #     self.current_model.set_adapter(merged_name)
